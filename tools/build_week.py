...
LESSON_BUILDER = ROOT / "tools" / "build_lesson.py"
GITHUB_BLOB_ROOT = (
...
def module_for_week(week: str) -> Path:
    number = int(week.split("-")[1])
    candidates = sorted(ROOT.glob(f"{number:02d}-*"))
    candidates = [
        path for path in candidates if path.is_dir() and (path / "overview.md").exists()
    ]
    if len(candidates) != 1:
        raise SystemExit(
            f"expected one topic folder matching {number:02d}-*, found "
            f"{len(candidates)}"
        )
    return candidates[0]
...
def lesson_with_challenge_links(
    ...
    for path in challenges:
        ...
        relative_pdf_url = f"repo-relative:challenges/{quote(path.name)}"
...
def main() -> None:
    ...
    module = module_for_week(args.week)
    overview = module / "overview.md"
    lessons = sorted((module / "sections").glob("day-*.md"))

    if not overview.exists():
        raise SystemExit(f"missing weekly overview: {overview}")
    if not lessons:
        raise SystemExit(f"no daily lesson sources found under {module / 'sections'}")

    number = int(args.week.split("-")[1])
    output = module / f"week-{number:02d}-material.pdf"
