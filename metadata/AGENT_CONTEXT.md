...
3. Update the consolidated rules in this file if the change affects future
   sessions.
4. Update `metadata/MASTERCLASS.md` only when learning progress or the broader
...
1. Create coherent lesson sections under the current numbered topic folder's
   `sections/` directory.
2. Render the consolidated PDF at the topic-folder root as
   `week-NN-material.pdf`.
3. Open the PDF in the editor.
4. When useful, create reinforcement under that topic's `challenges/`
   directory.
...
The user selected `metadata/curriculum-tracker.docx`
...
- Local library: `references/library/`
- Trackable catalog: `references/README.md`
...
renamed and organized under `references/library/training/kaiwantech/`:
...
`master` and `experiment/accelerated-foundations`. This was the initial
weekly experiment; its original `weekly/` directory was later superseded by
the learner-first numbered topic folders recorded below.
...
sources and labs into one weekly package. `tools/build_week.py` renders the
weekly overview plus all seven daily sections and merges them into one
topic-root `week-NN-material.pdf`. Keep daily source sections for
maintainability, but present one consolidated PDF to the learner.
...
The user requested challenge links inside the PDFs. `tools/build_week.py` now
...
embed machine-specific `file://` or `localhost` URLs. `tools/build_week.py`
...
`tools/generate_quizzes.py` regenerates the HTML. Treat quiz scores as
...
before authoring machinery. Official topic folders are:

- `01-foundations-to-cpu/`
- `02-assembly-memory-and-c/`
- `03-c-toolchain-and-startup/`

Each contains `week-NN-material.pdf`, `overview.md`, `sections/`,
`challenges/`, and `quizzes.json`. Supporting content lives under
`references/`, `tools/`, `metadata/`, and `archive/`. Future topic packages
should follow the same numbered-root convention. Keep PDF challenge links
relative to the topic folder (`challenges/<file>`).
