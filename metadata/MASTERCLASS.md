...
> `metadata/AGENT_CONTEXT.md` first. It contains the current
...
artifact: **`/home/jd/metadata/curriculum-tracker.docx`**
...
AUTHORITATIVE plan.** This metadata/MASTERCLASS.md file becomes the supporting
...
**Source of truth for ordering/scope:** `/home/jd/metadata/curriculum-tracker.docx`
...
doc = docx.Document("/home/jd/metadata/curriculum-tracker.docx")
...
by `metadata/WEEKLY_PROGRESS.md`. Earlier daily approaches are preserved on
...
- each module lives in a numbered topic folder such as
  `01-foundations-to-cpu/`;
- each topic contains `overview.md`, seven daily source sections under
  `sections/`, labs under `challenges/`, and `week-NN-material.pdf`;
- `metadata/WEEKLY_PROGRESS.md` stores the exact resume point;
- `tools/week` builds/opens the latest module, or `MC_WEEK=N tools/week` selects
  another.
